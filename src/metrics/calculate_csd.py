...
parser.add_argument('--dist-url', default='tcp://localhost:6001', type=str,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    style_images.sort()
    # np.random.shuffle(style_images)  # take 1 random style image from the pool
    ...
    elif args.dataset == 'nsync':
        dataset_query = NsyncD(predictions_path, ret_transform)
        dataset_values = NsyncD(style_images_dir, ret_transform)
    else:
    ...
    if args.rank == 0:  # only rank 0 will work from now on
        ...
        # calculate similarity, take the mean features from values and calculate cosine similarity between a query and the mean feature
        # cosine_similarities = []
        # mean_style_feature = l_values_features
        mean_style_feature = torch.mean(values_features, dim=0).unsqueeze(0)
        cos = nn.CosineSimilarity(dim=1, eps=1e-6)
        cosine_similarity = cos(mean_style_feature, query_features)
        
        print(torch.mean(cosine_similarity))
